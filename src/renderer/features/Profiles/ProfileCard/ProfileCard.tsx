        ...
        <div className={`card card-hover relative overflow-hidden ${isActive ? 'border-green-500/50' : ''}`}>
            ...
            <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-theme-text truncate">{profile.name}</h3>
                    <p className="text-sm text-theme-text-muted">{formatDate(profile.created_at)} · {formatTime(profile.created_at)}</p>
                </div>
            </div>
            ...
            <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-theme-text">{browserInfo.name}</span>
                </div>

                <div className="flex items-center gap-2 text-sm">
                    <span className="text-lg">🌍</span>
                    <span className="text-theme-text">{profile.fingerprint.timezone}</span>
                </div>
                ...
                {profile.proxy ? (
                    ...
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-lg">🔓</span>
                        <span className="text-theme-text-muted">Sem proxy</span>
                    </div>
                )}
            </div>
            ...
            <div className="flex items-center gap-2">
                {isActive ? (
                    ...
                    <button
                        onClick={onLaunch}
                        className="flex-1 py-2 rounded-lg gradient-primary text-theme-text hover:opacity-90 transition-opacity font-medium shadow-lg shadow-primary-500/20"
                    >
                        Abrir
                    </button>
                )}
                ...
                <button
                    onClick={onEdit}
                    className="p-2 rounded-lg bg-theme-card text-theme-text hover:bg-theme-card hover:text-theme-text transition-colors"
                    title="Editar"
                >
                ...
                <button
                    onClick={onDelete}
                    className="p-2 rounded-lg bg-theme-card text-theme-text hover:bg-red-500/20 hover:text-red-400 transition-colors"
                    title="Excluir"
                >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
