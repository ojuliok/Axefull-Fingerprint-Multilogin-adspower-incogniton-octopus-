...
import { MarketingSpace, MarketingFolder, MarketingBoard, MarketingGroup, MarketingCardData, fetchCrmGroups, fetchCrmCards, pushCrmGroupToSupabase, pushCrmCardToSupabase } from '../services/crmStorage';
import { useWorkspace } from '../../../context/WorkspaceContext';
import { getCanvasList, getCanvasData, saveCanvasData, CanvasInfo } from '../../../features/Canvas/canvasStorage';
...
export const CRMProvider: React.FC<{ children: ReactNode, forcedBoardId?: string }> = ({ children, forcedBoardId }) => {
    ...
    // DATA-002: Controle de debounce para atualizações contínuas de leads
    const debouncedLeadTimers = useRef<Record<string, NodeJS.Timeout>>({});
    const pendingLeadUpdates = useRef<Record<string, MarketingCardData>>({});

    const flushPendingLeadUpdate = (id: string) => {
        if (debouncedLeadTimers.current[id]) {
            clearTimeout(debouncedLeadTimers.current[id]);
            delete debouncedLeadTimers.current[id];
        }
        if (pendingLeadUpdates.current[id]) {
            pushCrmCardToSupabase(pendingLeadUpdates.current[id], 'update').catch(err => {
                console.error('[CRMContext] Falha ao persistir lead:', err);
            });
            delete pendingLeadUpdates.current[id];
        }
    };

    // Flush pendente ao trocar de lead selecionado ou desmontar
    useEffect(() => {
        if (selectedLeadId) {
            flushPendingLeadUpdate(selectedLeadId);
        }
        return () => {
            Object.keys(pendingLeadUpdates.current).forEach(id => flushPendingLeadUpdate(id));
        };
    }, [selectedLeadId]);
    ...
    const updateLead = (id: string, updates: Partial<MarketingCardData>, immediate = false) => {
        const updatedTimestamp = Date.now();
        const newLeads = leads.map(lead => lead.id === id ? { ...lead, ...updates, updatedAt: updatedTimestamp } : lead);
        setLeads(newLeads);
        const updated = newLeads.find(l => l.id === id);
        if (!updated) return;

        if (immediate) {
            flushPendingLeadUpdate(id);
            pushCrmCardToSupabase(updated, 'update');
        } else {
            pendingLeadUpdates.current[id] = updated;
            if (debouncedLeadTimers.current[id]) {
                clearTimeout(debouncedLeadTimers.current[id]);
            }
            debouncedLeadTimers.current[id] = setTimeout(() => {
                flushPendingLeadUpdate(id);
            }, 300);
        }
    };
